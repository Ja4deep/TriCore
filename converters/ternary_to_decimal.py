...
def place_value_table_lines(ternary: str) -> list[str]:
    ...
    rows = [
        ...
        for digit, value, power, place_value, contribution in build_place_value_rows(ternary)
    ]
...
def display_place_value_table(ternary: str) -> None:
    ...
    print_screen(explanation_screen("Place-Value Table", place_value_table_lines(ternary)))
...
def display_results(ternary: str) -> None:
    ...
    print_screen(explanation_screen("Verification", verify_conversion(normalized_value).splitlines()))
    print_screen(
        explanation_screen(
            "Learning Summary",
            [
